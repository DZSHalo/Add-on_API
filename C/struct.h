...
struct chatData    {
        unsigned int type;     //range of 0 - 3, sort from Global, Team, Vehicle, and Server (CE only)
...
static_assert_check(sizeof(s_ident) == 0x4, "Incorrect size of s_ident");
struct bone {
    float unknown[10];
    vect3 World;
};
...
struct s_machine_slot {
    ...
    //NOTICE: This below is disabled as it is no inter needed.
...
struct s_biped {
    ...
    unsigned char    actionVehicle_crouch_stand;    // 0x2A0 Is this really true? Found this from Wizard's code (Standing = 4) (Crouching = 3) (Vehicle = 0)
